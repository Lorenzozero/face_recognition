...
def _make_radar_chart(sources: dict) -> io.BytesIO:
    """Radar chart delle fonti OSINT con i loro score."""
...
def _make_social_bar(platforms: list) -> io.BytesIO:
    """Bar chart orizzontale dei profili social trovati."""
    ...
    for i, (bar, f) in enumerate(zip(bars, found)):
        ...
        ax.text(0.05, i, label, va="center", ha="left",
...
def _make_pipeline_chart() -> io.BytesIO:
    ...
    for i, (x, txt, col) in enumerate(steps):
        circ = plt.Circle((x, 1.5), 0.55, color=(*hexrgb(col), 0.2),
                           ec=col, lw=2, zorder=3)
        ax.add_patch(circ)
        ax.text(x, 1.5, txt, ha="center", va="center",
                color="#f0f6fc", fontsize=7, fontweight="bold",
                linespacing=1.4, zorder=4)
        if i < len(steps)-1:
            ax.annotate("", xy=(steps[i+1][0]-0.57, 1.5),
                        xytext=(x+0.57, 1.5),
...
def build_pdf(
    ...
    Args:
        osint_data: dizionario restituito da /osint/full
    ...
    # Immagine target + KPI stats fianco a fianco
    kpi_items = [
        ("Volti rilevati", str(faces),  CYAN),
        ("Fonti OSINT",   str(len(reverse)), ORANGE),
        ("Profili social",str(len(social) if isinstance(social, list) else len(social.get("platforms",[]))), GREEN),
    ...
    story.append(Paragraph(
        ...
        "I link seguenti aprono direttamente la ricerca con il volto ritagliato.",
        sBody))

    if reverse:
        ri_rows = [["Motore", "URL di Ricerca", "Stato"]]
        for engine, url in reverse.items():
            if engine in ("faces_detected", "face_cropped"):
                continue
            ri_rows.append([
                engine.replace("_"," ").title(),
                Paragraph(f'<font size="7" color="#00e5ff">{str(url)[:80]}</font>', sBody) if url else "N/A",
                Paragraph('<font color="#00ff9f">Disponibile</font>', sBody) if url else
                Paragraph('<font color="#ef233c">Errore</font>', sBody),
            ])
        ri_t = Table(ri_rows, colWidths=[35*mm, 100*mm, 25*mm])
        ri_t.setStyle(_dark_table(ORANGE))
        story.append(ri_t)
    else:
    ...
    # Radar chart efficacia fonti
    source_scores = {
        "Google Lens": 90, "Yandex": 85, "TinEye": 75,
        "PimEyes": 95, "Bing Visual": 70,
    }
    ...
    if platforms_list:
        ...
        story.append(RLImage(bar_buf, width=W-34*mm,
                              height=max(35*mm, len(platforms_list)*8*mm)))
    ...
    # Varianti username generate
    story.append(Paragraph("3.1  Varianti username generate", sH2))
    if variants:
        var_text = "  |  ".join(variants)
        story.append(Paragraph(
            f'<font name="Courier" color="#00ff9f">{var_text}</font>', sBody))
    else:
    ...
    if maigret:
        ...
        if isinstance(maigret, dict):
            for username, sites in maigret.items():
                ...
                elif isinstance(sites, dict) and "sites" in sites:
                    for site in sites["sites"]:
                        found_sites.append({"username": username, **site})

        if found_sites:
            mg_rows = [["Username", "Sito", "URL", "Status"]]
            for s in found_sites[:30]:  # max 30 risultati per non sforare pagina
                mg_rows.append([
                    s.get("username", ""),
                    s.get("site", ""),
                    Paragraph(f'<font size="6.5" color="#00e5ff">{s.get("url","—")[:55]}</font>', sBody),
                    Paragraph('<font color="#00ff9f">OK</font>', sBody)
                    if s.get("status") in ("found", 200, "200")
                    else Paragraph('<font color="#8b949e">—</font>', sBody),
                ])
            mg_t = Table(mg_rows, colWidths=[28*mm, 35*mm, 80*mm, 18*mm])
            mg_t.setStyle(_dark_table(CYAN))
            story.append(mg_t)
        else:
            story.append(Paragraph(
                "Maigret eseguito ma nessun profilo confermato.", sWarn))
    else:
        story.append(Paragraph(
            "Maigret non eseguito (run_maigret=False o non installato).", sWarn))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Genera un PDF OSINT da un file JSON di risultati")
    parser.add_argument("--data", required=True,
                        help="Path al file JSON con i dati OSINT")
    parser.add_argument("--image", default=None,
                        help="Path opzionale all'immagine del target")
    parser.add_argument("--out", default="osint_report.pdf",
